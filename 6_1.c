...
typedef struct simple_metadata_t {
  size_t size;
  struct simple_metadata_t* next;//次の領域についても入れ子構造体で定義
} simple_metadata_t;
...
typedef struct simple_heap_t {
  simple_metadata_t* free_head;//ポインタ
...
void simple_add_to_free_list(simple_metadata_t* metadata) {
  assert(!metadata->next); //metadataのnextが空でなければ,プログラムの実行を停止
  metadata->next = simple_heap.free_head;//freeなメモリの先頭を次のmetadataのポインタに格納
  simple_heap.free_head = metadata;//metadataをヘッドのポインタで指せるように
}
...
void simple_remove_from_free_list(simple_metadata_t* metadata,
  ...
  if (prev) {
    prev->next = metadata->next;//prevが空でないならmetadataを格納移動
  } else {
    simple_heap.free_head = metadata->next;//freeなメモリの先頭を指す
  }
  metadata->next = NULL;
}
...
void simple_initialize() {
  simple_heap.free_head = &simple_heap.dummy;//ヒープ領域の最初のアドレスを格納
...
void* simple_malloc(size_t size) {
  simple_metadata_t* metadata = simple_heap.free_head;//freeの先頭で初期化
  simple_metadata_t* prev = NULL;
  // First-fit: Find the first free slot the object fits.
  while (metadata && metadata->size < size) {//metadataが存在して、そのサイズが空領域より小さいなら
    prev = metadata;//metadataを格納
    metadata = metadata->next;//更新
  }
  
  if (!metadata) {//metadataがNULLなら
  ...
  //     ^          ^
  //     metadata   ptr
  void* ptr = metadata + 1;//metadataには確保された領域へのポインタが格納される
  size_t remaining_size = metadata->size - size;//割り当ての後の残りメモリ量
  metadata->size = size;
  // Remove the free slot from the free list.
  simple_remove_from_free_list(metadata, prev);//metadataからprevを除去
  
  if (remaining_size > sizeof(simple_metadata_t)) {//残りのメモリが十分ならmetadata確保
...
//freelistをサイズの要素で昇順に並べ替えて、そこにinsertしていく形で昇順のfreelistを運用
//my_initiallizeでfreelistを昇順に並べかえ
//simple_add_to_free_listを呼び出しているところで、サイズが昇順に並んでいるのを壊さないよう、
//適切なところに挿入するような関数を実行(simple_add_to_free_listを書き換え)
//list_swapをすでに存在しているfreelistに適用する方法が分からなかったです
typedef struct Cell{
  struct Cell* prev,next;
  size_t size;
}Cell;

void list_swap(struct Cell* y){
  //x->y->z->wを
  //x->z->y->wにする
  struct Cell* z = y->next;
  struct Cell* x = y->prev;
  struct Cell* w = z->next;
  x->next=z;
  z->prev=x;
  z->next=y;
  y->prev=z;
...
void* my_malloc(size_t size) {
  simple_metadata_t* metadata = simple_heap.free_head;//freeの先頭で初期化
  simple_metadata_t* prev = NULL;
  Cell* = simple_metadata_t* metadata;
  Cell* prev = NULL;
  // First-fit: Find the first free slot the object fits.
  while (metadata && metadata->size < size) {//metadataが存在して、そのサイズが空領域より小さいなら
    prev = metadata;//metadataを格納
