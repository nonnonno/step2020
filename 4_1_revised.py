...
def bfs(source,dest,data_lst):
    ...
    register_lst = []
    register_lst_tmp = []
    register_lst.append(source)
    register_lst_tmp.append(source)
    ...
    while depth < max_depth:#せいぜいノードの種類分の回数探索すればよい
        ...
        while index < parent_lst_len:#親になる層のノードの個数回探索
            ...
            for node in child(parent_lst[index],data_lst):#返ってきた子ノードのリストが探索済みか調べる
                register_lst_tmp.append(node)#nodeをつけてみる
                if len(register_lst) != len(set(register_lst_tmp)):#nodeが未探索なら
                    tmp_lst.append(node)#次に探索する親ノードのリストに追加
                    register_lst.append(node)
